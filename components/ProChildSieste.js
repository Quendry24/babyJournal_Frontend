...
export default function ProChildSieste({ childInfo }) {
  const [selectedItem, setSelectedItem] = useState(null);
  const [commentaire, setCommentaire] = useState(null);
  const [heureCouche, setHeureCouche] = useState(new Date());
  const [heureReveil, setHeureReveil] = useState(new Date());
  const [ok, setOk] = useState(false);

  const save = async () => {
    const couche = heureCouche.toLocaleTimeString("fr-FR", {
      hour: "2-digit",
      minute: "2-digit",
    });
    const reveil = heureReveil.toLocaleTimeString("fr-FR", {
      hour: "2-digit",
      minute: "2-digit",
    });
    ...
    try {
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_URL_BACKEND}/nounou/Siestes/${childInfo.idBabyJournal}`,
        {
          method: "POST",
      ...
      if (data.result) {
        console.log("sieste ajouté");
        setOk(true);
        setHeureCouche(new Date());
        setHeureReveil(new Date());
      ...
      >
        <View className=" gap-2 ">
          <View className="flex-row items-center gap-4">
            <Text className="text-2xl">Heure du couché :</Text>
            <RNDateTimePicker
              mode="time"
              value={heureCouche}
              onChange={(event, selectedDate) => {
                if (selectedDate) {
                  setHeureCouche(selectedDate);
                }
              }}
            />
          </View>
          <View className="flex-row items-center gap-4">
            <Text className="text-2xl">Heure du reveil :</Text>
            <RNDateTimePicker
              mode="time"
              value={heureReveil}
              onChange={(event, selectedDate) => {
                if (selectedDate) {
                  setHeureReveil(selectedDate);
                }
              }}
            />
          </View>
